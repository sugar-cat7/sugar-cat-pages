...
describe("Logger", () => {
  describe("ロガー作成", () => {
    const testCases = [
      {
        name: "デフォルト設定でロガーを作成する",
        config: undefined,
      },
      {
        name: "カスタム設定でロガーを作成する",
        config: {
          name: "test-logger",
          minLevel: 2,
  ...
  describe("ログメソッド", () => {
    const testCases = [
      ...
      {
        name: "dataパラメータを受け入れる",
        action: (logger: Logger) => {
          expect(() => logger.info("test", { userId: "123" })).not.toThrow();
        },
      },
      {
        name: "errorメソッドがErrorオブジェクトを受け入れる",
        action: (logger: Logger) => {
      ...
      {
        name: "fatalメソッドがErrorオブジェクトを受け入れる",
        action: (logger: Logger) => {
  ...
  describe("withContext", () => {
    const testCases = [
      ...
      {
        name: "コンテキストロガーのerrorがErrorオブジェクトを受け入れる",
        action: (logger: Logger) => {
          const contextLogger = logger.withContext({ traceId: "test-trace" });
          const error = new Error("Test error");
          expect(() =>
