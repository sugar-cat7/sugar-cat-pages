...
> 参照: [You Might Not Need an Effect – React](https://react.dev/learn/you-might-not-need-an-effect)
...
function useInterviewData(sessionId: string) {
  return useQuery({
    queryKey: ["interview", sessionId],
    queryFn: () => fetchInterviewData(sessionId),
  });
}
```
